...
Delta 60 Ltd is a company registered in England and Wales (company number 17049123). Registered office: Lytchett House, Unit 13 Freeland Park, Wareham Road, Lytchett Matravers, Poole, BH16 6FA. You can reach us through our contact form or by emailing hello@delta60.com.
...
For security and fraud-prevention purposes, where you provide a professional registration body and registration number, we may electronically check the registration details against that body's public register to confirm the registration is valid.

If we are unable to confirm a registration number, or if we identify it as invalid or being used in a way that suggests fraud or abuse, we may restrict access to the app. These checks involve human review and are not based solely on automated processing.
...
If you use the Jobs tab to find roles near you, the app may use your device location, with your permission, to filter and sort job listings. On Android we request approximate location only. On iOS we use a lower-accuracy foreground location request where available, but the actual precision may also depend on your device settings. Your location is used only while you are using the Jobs feature. We do not request background location access, and we do not save your device location to your Learning Log account. Your latitude and longitude may be sent to our backend temporarily to calculate nearby jobs and distances. You can decline location permission and search by postcode instead.

The map in the Jobs feature is rendered using MapTiler (api.maptiler.com), which serves the map tiles and styles (built from OpenStreetMap data). To draw the map, your device requests these tiles directly from MapTiler, so MapTiler receives your device's IP address and the map area you are viewing. We do not send your name, email, or Learning Log account details to MapTiler. OpenStreetMap is credited as the underlying map data source within the map view.
...
- To verify professional registration details you provide against the relevant public regulator's register.

- To identify accounts held by the same person, so we can enforce the one-account-per-person limit in our Terms of Service and prevent abuse of welcome bonuses or other limits.
...
- **Reading logs and event logs:** kept while your account is active for as long as the entry stays in your portfolio. When you generate a PDF from an entry, that entry is archived and is then deleted automatically 24 months later. Entries you never turn into a PDF are not deleted on a timer.

- **Generated outputs (PDFs):** kept for up to 24 months from the date they were created, then deleted automatically. This applies whether your account is active or not.
...
Delta 60 Ltd · Lytchett House, Unit 13 Freeland Park, Wareham Road, Lytchett Matravers, Poole, BH16 6FA · Company No. 17049123 · hello@delta60.com
